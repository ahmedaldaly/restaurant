...
const Page = () => {
  ...
  const filterByCategory = async (categoryName: string) => {
    ...
    try {
      const res = await axios.get(
        `${BaseUrl}/api/v1/product/by-category?category=${categoryName}`
      );
      setFilteredProducts(res.data || []);
    } catch (err) {
      console.error(err);
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    axios
      ...
      .then(() => {
        toast.success(
          Arabic
            ? "تم الحجز، انتظر التأكيد"
            : "Booking confirmed, please wait for approval"
        );
      })
      .catch(() => {
        setErrorMessage(
          Arabic ? "حدث خطأ أثناء الحجز" : "An error occurred during booking"
        );
      })
      .finally(() => {
        setLoading(false);
  ...
  const addOrder = async (id: string) => {
    try {
      ...
      toast.success(
        Arabic
          ? "تم الطلب انتظر التأكيد"
          : "Order confirmed, please wait for approval"
      );
    } catch (err) {
    ...
    <PageWrapper>
      <div>
        ...
        <motion.div
          ...
          className="flex justify-center gap-3 my-6 flex-wrap"
        >
          <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {filteredProducts.map((item, i) => (
                <motion.div
                  ...
                  key={item._id}
                  className="bg-white dark:bg-gray-700/50 rounded-2xl shadow-md p-4 hover:shadow-lg transition-all"
                >
